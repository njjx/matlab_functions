function sgm=XuGenSgmOriginalSizeNoDeadPixelCorrHeadCTNew(s_prj,s_bkg,s_config)
...
%the reason we do not apply the dead pixel correction is that it takes
%time.
...
img_bkg=readHydraEviFromDirectNew(s_bkg,recon_para.BkgNum,begin_frame_idx);

img_bkg=mean(img_bkg,3);
img_bkg=imresize(img_bkg(:,3:58),[5120 recon_para.SliceCount]);
disp(['Reading ' s_prj ' ...']);
...
for idx=1:recon_para.SinogramHeight
    ...
    img_temp=readHydraEviFromDirectNew(s_prj,1,idx+begin_frame_idx-1);
